...
def get_goal_data(patient_id):
    ...
    for it in patient_response["entry"]:
        ...
        goalsOut[key]=str(low_val)+"-"+str(high_val)
    return goalsOut
...
def insert_goal_data(patient_id,data):
    ...
    for key in data.keys():
        ...
        sampleDict["measure"]["coding"]=[{"system": "http://loinc.org","code": "3141-9"}]
        sampleDict["detailRange"]["low"]["value"]=float(data[key].split("-")[0].strip())
        sampleDict["detailRange"]["low"]["unit"]=units
        sampleDict["detailRange"]["high"]["value"]=float(data[key].split("-")[-1].strip())

...
class GraphGen(Resource):
    def get(self, id):
        ...
        df.effective_date = df.effective_date.apply(lambda x: dt.strptime(
            x.replace("T", " ").split("+")[0], '%Y-%m-%d %H:%M:%S').timestamp()*1000)
        resutlDict = {}
        for value in set(df.value_code):
            df_temp = df[df.value_code == value]
            resutlDict[value] = [[int(i), j] for i, j in zip(
                list(df_temp.effective_date), list(df_temp.value))]
        return resutlDict
